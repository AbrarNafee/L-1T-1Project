...
#define pointnum -1
#define minenumber 4
#define dekha printf("Dhuktese\n");
double lvlend[3]={1690,2460};
int linestart[5]={0,12},lineend[5]={11,27},obsstart[5]={0,8},obsend[5]={8,16},danstart[5]={0,7},danend[5]={7,15};
...
int obst[obsnum], dancon[dangernum];
int highscore = 0, prevlevel, menu=1, sound=1, ft=1, life=3, uacon=0, ballposition, charcon=0, level=-4, playerlevel;
int intronum=1, introtimer=0, jump=0, namelen=0,ldx=2,ldy=2,completed=0;
int i,cp,p,ballfly,frontindex=0,knockout=0,index=0,backindex=7,hangindex=0,standcounter=0,run=0,extralife=1,flag=0,leveldekhao=0;
...
char charbmpname[200], playername[20];
...
char firepic[18][40]={"project image\\fire 1.bmp","project image\\fire 1.bmp","project image\\fire 1.bmp","project image\\fire 2.bmp","project image\\fire 2.bmp",
                     "project image\\fire 2.bmp","project image\\fire 3.bmp","project image\\fire 3.bmp","project image\\fire 3.bmp","project image\\fire 4.bmp",
                     "project image\\fire 4.bmp","project image\\fire 4.bmp","project image\\fire 5.bmp","project image\\fire 5.bmp","project image\\fire 5.bmp",
                     "project image\\fire 6.bmp","project image\\fire 6.bmp","project image\\fire 6.bmp"};
char minepic[2][40]={"project image\\minecraft 2.bmp","project image\\minecraft 1.bmp"};
...
void dangers(){
    ...
    danger(9, 1800, 220, 30, 30, 4, 0, 230);///moving danger
    danger(10, 1200, 320, 25, 25, 101, 1);///life++
    danger(11, 826, 254, 30, 30);
    danger(12, 1734, 412, 30, 30);
    danger(13 ,2228, 324, 30, 30);
    danger(14, 2437, 324, 30, 30);
...
void ko()
{
    if(level==3){
        ...
        if(index>29)  {  index=0;  knockout=0;  restart();  }
    }
    else if(knockout==2){
        index++;
        if(index>=1 && index <=9)  {  ballx1+=6;  bally1+=5;  }
...
void ball_position(){
    ...
    if(level==1 || level==2){
        ...
        if(bally> _line_y[ballposition] && uacon    == 0)
            falling+=.025;
//            uacon=1;
        else
            falling =0;
...
void obstacle(int i, double x, double y, double l, double b, int type=1){
    ...
    iSetColor(255, 255, 0);
    if(type==1){
        //iFilledRectangle(pox+x, y, l, b);
//        iShowBMP(pox+x, y, "Games//box.bmp");
    }
}
void obstacles(){
    ...
    //obstacle(6, 200, 350+150, 50, 50, 1);
    ...
    obstacle(13,1500,220,350,190,2);
...
int obstacle_check(double cx, double cy){
    if(level==1 || level==2){
        for(int i=obsstart[level-1]; i<obsend[level-1]; i++){
            ...
            //printf("\tobs: %lf %lf, %lf -- %lf, %lf %lf\n", cx, cy, obsx[i], obsx[i]+obsl[i], obsy[i], obsb[i]+obsy[i]);
                return 0;
            }
        }
        return 1;
...
void charname(){
    ...
    if(charcon==0)
        strcat(charbmpname, "s//");
    if(charcon==1 || charcon==2)
        strcat(charbmpname, "r//");
    if(charcon==3 || charcon==4)
        strcat(charbmpname, "j//");
    else
        strcat(charbmpname, "s//");
...
void leaderboard(){
	...
	//char playername[40];
	int found=0;
	//scanf("%s %d", playername, &playerlevel);
	while (1) {
        //printf("dhuktese");
		char tmpname[40];
		int tmplevel;
    ...
    if(level==1 || level==2){
        ...
        for(int i=linestart[level-1];i<lineend[level-1];i++){
            ...
            if(_line_t[i]==3){
                    ...
                    //iFilledRectangle(pox+_line_x[i],poy+_line_y[i],_line_l[i],_line_b[i]);
            }
        }
        iSetColor(0, 255, 0);
        iRectangle(850, 650, 99, 20);
        ...
        iFilledRectangle(850, 652, 33*life, 16);
        for(int i=danstart[level-1]; i<danend[level-1]; i++){
            if(dant[i]==1)iShowBMP2(pox+danx[i], poy+dany[i], "Games//grenade.bmp", 0xffffff);
...
//        if(level==1)
        ...
        //iFilledCircle(ballx+pox, bally+poy+5, ballr);
        iShowBMP2(ballx+pox-28, bally+poy-ballr/2, charbmpname, 0xffffff);
        //iFilledCircle(ballx+pox, bally+poy, 5);
    }
    iSetColor(0,255,0);
    if(level==4) iText(400,350,"Level 4 is yet to be build. Thank you!");
...
void iDraw(){
    ...
    if(level==-6){
        iText(100, 100, "Info is being added");
        iShowBMP2(0, 650, "Games//Menu//Menubar.bmp", 0xffffff);
    }
...
void iKeyboard(unsigned char key){
    if(key == 'c' || key == 'C') printf("\tuacon: %d\n", uacon);
    ///notun ashche///
    if(key == 'a' || key=='A') iSpecialKeyboard(GLUT_KEY_LEFT);
    if(key == 's' || key=='S') iSpecialKeyboard(GLUT_KEY_DOWN);
    if(key == 'd' || key=='D') iSpecialKeyboard(GLUT_KEY_RIGHT);
    if(key == 'W' || key=='w') iSpecialKeyboard(GLUT_KEY_UP);
    if(level==0 && key==13){
            level=playerlevel;
            if(level==1 || level==2){
                main_menu=false; pause=false; level_12=true; level_3=false;
...
int main(){
    ...
    //iSetTimer(10,dangertimer);
    iSetTimer(10, timer);
    iSetTimer(10, gravity);
    iSetTimer(100, charcondition);
    iSetTimer(20,change);  iSetTimer(20,change1);  iSetTimer(20,minechange);  iSetTimer(5,velocity); iSetTimer(80,ko); iSetTimer(50,checkshow);
    if(main_menu){
        PlaySound("Musix\\main menu.wav",NULL,SND_LOOP | SND_ASYNC);
    }
